...
mort2016 <- read.csv("../Life expectancy/AF_mort2016.csv",h=T)
...
ggsave("../Life expectancy/Smoking mortality.png",dpi=300,height = 12,width = 16)
#--------------------------------------------------------------------------------------------------------------------------------------------------------
# Load smoking attributable fraction data
af2016 <- read.csv("../Life expectancy/AF2016.csv",h=T)
...
ggsave("../Life expectancy/Smoking AF.png",dpi=300,height = 12,width = 16)
...
load('../Life expectancy/DeathData7618.rda')
...
kor_deaths <- data.table::fread("../Life expectancy/Deaths_1x1 Korea.txt") %>% data.frame()
...
kor_exposure <- data.table::fread("../Life expectancy/Exposures_1x1 Korea.txt") %>% data.frame()
...
mx_overall$Country <- factor(mx_overall$Country,levels = c("Hong Kong","18 OECD high-income countries","Australia","Japan","Singapore","South Korea","UK","USA"))

ggplot(mx_overall %>% filter(Year<2017) %>% na.omit(),aes(Year,mx_smoking*100000,color=Country)) + geom_line(lwd = 1.2) +
  facet_wrap(~Sex) + theme_classic() + scale_x_continuous(breaks = c(seq(1980,2010,10))) + 
...
ggsave("../Life expectancy/Smoking AF mx.png",dpi=300,height = 16.5,width = 22)
...
saf_smk <- read.csv("../Life expectancy/AF_overall_individual_0303_for CIs.csv",h=T)
...
smokingAF <- read.csv("../Life expectancy/AF_overall_FINAL_LIMOR_timevarying_0128.csv",h=T)
...
write.csv(.,"../Life expectancy/Smoking AF mx 2016 .csv",row.names = F)
